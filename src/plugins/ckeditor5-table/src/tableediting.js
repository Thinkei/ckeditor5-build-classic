...
export default class TableEditing extends Plugin {
	...
	init() {
		...
		conversion
			.for('editingDowncast')
			.add(downcastInsertTable({ asWidget: false }));
		conversion.for('dataDowncast').add(downcastInsertTable());

		// Table row conversion.
		conversion
			.for('upcast')
			.add(upcastElementToElement({ model: 'tableRow', view: 'tr' }));

		conversion
			.for('editingDowncast')
			.add(downcastInsertRow({ asWidget: false }));
		...
		conversion
			.for('editingDowncast')
			.add(downcastInsertCell({ asWidget: false }));
		...
		conversion
			.for('editingDowncast')
			.add(downcastTableHeadingColumnsChange({ asWidget: false }));
		conversion.for('dataDowncast').add(downcastTableHeadingColumnsChange());
		conversion
			.for('editingDowncast')
			.add(downcastTableHeadingRowsChange({ asWidget: false }));
		...
		editor.commands.add(
			...
			new InsertColumnCommand(editor, { order: 'before' })
		);
		editor.commands.add(
			'insertTableColumnAfter',
			new InsertColumnCommand(editor, { order: 'after' })
		);
		...
		this.listenTo(editor.editing.view.document, 'keydown', (...args) =>
			this._handleTabOnSelectedTable(...args)
		);
		this.listenTo(editor.editing.view.document, 'keydown', (...args) =>
			this._handleTabInsideTable(...args)
		);
	}
	...
	_handleTabOnSelectedTable(eventInfo, domEventData) {
		...
		if (
			...
			selection.getFirstRange().isFlat
		) {
			...
			editor.model.change(writer => {
				writer.setSelection(
					Range.createIn(selectedElement.getChild(0).getChild(0))
				);
			});
		}
	}
	...
	_handleTabInsideTable(eventInfo, domEventData) {
		...
		if (isForward && isLastRow && isLastCellInRow) {
			editor.plugins
				.get(TableUtils)
				.insertRows(table, { at: table.childCount });
		}
