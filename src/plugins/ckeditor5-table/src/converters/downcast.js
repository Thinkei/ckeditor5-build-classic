...
import { toTableWidget, isTableWidget } from '../utils';
import './styles.css';
		...
		dispatcher.on(
			...
			(evt, data, conversionApi) => {
				...
				for (const tableWalkerValue of tableWalker) {
					...
					const tableSection = getOrCreateTableSection(
						getSectionName(row, tableAttributes),
						tableElement,
						conversionApi
					);
					...
					const insertPosition = ViewPosition.createAt(
						trElement,
						'end'
					);

					createViewTableCellElement(
						tableWalkerValue,
						tableAttributes,
						insertPosition,
						conversionApi,
				...
				conversionApi.mapper.bindElements(
					table,
					asWidget ? tableWidget : figureElement
				);
				conversionApi.writer.insert(
					viewPosition,
					asWidget ? tableWidget : figureElement
			...
			(evt, data, conversionApi) => {
				...
				for (const tableWalkerValue of tableWalker) {
					if (tableWalkerValue.cell === tableCell) {
						const trElement = conversionApi.mapper.toViewElement(
							tableRow
						);
						const insertPosition = ViewPosition.createAt(
							trElement,
			...
			(evt, data, conversionApi) => {
				...
				if (newRows > oldRows) {
					...
					const rowsToMove = Array.from(table.getChildren()).filter(
						({ index }) => isBetween(index, oldRows - 1, newRows)
					);

					const viewTableHead = getOrCreateTableSection(
						'thead',
						viewTable,
						conversionApi
					);
					moveViewRowsToTableSection(
						rowsToMove,
						viewTableHead,
						conversionApi,
						'end'
	...
	return dispatcher =>
		dispatcher.on(
			...
			(evt, data, conversionApi) => {
				...
				const lastColumnToCheck =
					(oldColumns > newColumns ? oldColumns : newColumns) - 1;
	...
	return dispatcher =>
		dispatcher.on(
			...
			(evt, data, conversionApi) => {
				...
				const viewStart = conversionApi.mapper
					.toViewPosition(data.position)
					.getLastMatchingPosition(value => !value.item.is('tr'));
				...
				if (!tableSection.childCount) {
					...
					conversionApi.writer.remove(
						ViewRange.createOn(tableSection)
					);
				}
			},
			{ priority: 'higher' }
...
function renameViewTableCell(
	tableCell,
	desiredCellElementName,
	conversionApi,
	asWidget
) {
	...
	if (asWidget) {
		const editable = conversionApi.writer.createEditableElement(
			desiredCellElementName,
			viewCell.getAttributes()
		);
		renamedCell = toWidgetEditable(editable, conversionApi.writer);

		conversionApi.writer.insert(
			ViewPosition.createAfter(viewCell),
			renamedCell
		);
		conversionApi.writer.move(
			ViewRange.createIn(viewCell),
			ViewPosition.createAt(renamedCell)
		);
		conversionApi.writer.remove(ViewRange.createOn(viewCell));
	} else {
		renamedCell = conversionApi.writer.rename(
			viewCell,
			desiredCellElementName
		);
	}
...
// @param {Object} conversionApi
// @param {Boolean} asWidget
function renameViewTableCellIfRequired(
	tableWalkerValue,
	tableAttributes,
	conversionApi,
	asWidget
	...
	const desiredCellElementName = getCellElementName(
		tableWalkerValue,
		tableAttributes
	);
...
function createViewTableCellElement(
	...
	const asWidget = options && options.asWidget;
	const cellElementName = getCellElementName(
		tableWalkerValue,
		tableAttributes
	);
...
function getOrCreateTableSection(sectionName, viewTable, conversionApi) {
	const viewTableSection = getExistingTableSectionElement(
		sectionName,
		viewTable
	);
	...
	return viewTableSection
		? viewTableSection
		: createTableSection(sectionName, viewTable, conversionApi);
}
...
function createTableSection(sectionName, tableElement, conversionApi) {
	...
	conversionApi.writer.insert(
		ViewPosition.createAt(
			tableElement,
			sectionName == 'tbody' ? 'end' : 'start'
		),
		tableChildElement
	);
...
function removeTableSectionIfEmpty(sectionName, tableElement, conversionApi) {
	const tableSection = getExistingTableSectionElement(
		sectionName,
		tableElement
	);
...
function moveViewRowsToTableSection(
	rowsToMove,
	viewTableSection,
	conversionApi,
	offset
	...
	for (const tableRow of rowsToMove) {
		...
		conversionApi.writer.move(
			ViewRange.createOn(viewTableRow),
			ViewPosition.createAt(viewTableSection, offset)
		);
	}
}
