		...
		while (
			...
			if (!removeStart.isEqual(removeEnd)) {
				const removed = viewWriter.remove(
					new ViewRange(removeStart, removeEnd)
				);
				lists.push(removed);
			}
...
export function viewModelConverter(evt, data, conversionApi) {
	if (conversionApi.consumable.consume(data.viewItem, { name: true })) {
		...
		writer.setAttribute(
			'parentStyles',
			data.viewItem.parent._styles,
			listItem
		);
		writer.setAttribute(
			'parentAttributes',
			data.viewItem.parent._attrs,
			listItem
		);
		writer.setAttribute('style', data.viewItem._styles, listItem);
		writer.setAttribute('class', data.viewItem._classes, listItem);
		...
		for (const child of data.viewItem.getChildren()) {
			...
			if (child.name == 'ul' || child.name == 'ol') {
				nextPosition = conversionApi.convertItem(child, nextPosition)
					.modelCursor;
			}
			// If it was not a list it was a "regular" list item content. Just convert it to `listItem`.
				...
				conversionApi.convertItem(
					child,
					ModelPosition.createAt(listItem, 'end')
				);
			}
		}
	...
	if (conversionApi.consumable.test(data.viewItem, { name: true })) {
		...
		for (const child of children) {
			...
			if (child.is('text')) {
				...
				if (
					!child.nextSibling ||
					(child.nextSibling.is('ul') || child.nextSibling.is('ol'))
				) {
					child._data = child.data.replace(/\s+$/, '');
				}
...
export function modelToViewPosition(evt, data) {
	...
	if (modelItem && modelItem.is('listItem')) {
		...
		const topmostViewList = viewItem
			.getAncestors()
			.find(element => element.is('ul') || element.is('ol'));
...
export function viewToModelPosition(evt, data) {
	...
	if (viewParent.name == 'ul' || viewParent.name == 'ol') {
		...
		if (!viewPos.isAtEnd) {
			...
			data.modelPosition = ModelPosition.createBefore(
				modelNode
		...
		data.modelPosition = ModelPosition.createBefore(modelNode).getShiftedBy(
			modelLength
		);
	...
	function _addListToFix(position) {
		...
		if (!prev || !prev.is('listItem')) {
			...
			while (
				...
				listHead.previousSibling.is('listItem')
			) {
...
function generateLiInUl(modelItem, conversionApi) {
	...
	const viewWriter = conversionApi.writer;
	const listType =
		modelItem.getAttribute('listType') == 'numbered' ? 'ol' : 'ul';
	const viewItem = createViewListItemElement(viewWriter);
	const viewList = viewWriter.createContainerElement(listType, null);
	const parentStyles = modelItem.getAttribute('parentStyles');
...
function getSiblingListItem(modelItemOrPosition, options) {
	...
	const indent =
		modelItemOrPosition instanceof ModelElement
			? modelItemOrPosition.getAttribute('listIndent')
			: options.listIndent;
	let item =
		modelItemOrPosition instanceof ModelElement
			? modelItemOrPosition.previousSibling
			: modelItemOrPosition.nodeBefore;
...
function injectViewList(modelItem, injectedItem, conversionApi) {
	...
	let insertPosition = mapper.toViewPosition(
		ModelPosition.createBefore(modelItem)
	);
	...
	const refItem = getSiblingListItem(modelItem, {
		sameIndent: true,
		smallerIndent: true
	});
	...
	if (
		...
		insertPosition = viewWriter.breakContainer(
			ViewPosition.createAfter(viewItem)
		);
	} else {
		...
		if (prevItem && prevItem.name == 'listItem') {
			...
			insertPosition = mapper.toViewPosition(
				ModelPosition.createAt(prevItem, 'end')
			);
		} else {
		...
		for (const value of walker) {
			if (value.item.is('li')) {
				const breakPosition = viewWriter.breakContainer(
					ViewPosition.createBefore(value.item)
				);
				const viewList = value.item.parent;

				const targetPosition = ViewPosition.createAt(
					injectedItem,
					'end'
				);
				mergeViewLists(
		...
		if (nextViewList && (nextViewList.is('ul') || nextViewList.is('ol'))) {
			...
			if (lastSubChild) {
				viewWriter.breakContainer(
					ViewPosition.createAfter(lastSubChild)
				);
				viewWriter.move(
					ViewRange.createOn(lastSubChild.parent),
					ViewPosition.createAt(injectedItem, 'end')
				);
			}
		}
	}
